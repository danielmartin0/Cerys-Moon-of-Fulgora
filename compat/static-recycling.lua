...
if settings.startup["cerys-enforce-vanilla-recycling-recipes"].value then
	...
	data.raw["recipe"]["copper-cable-recycling"]["energy_required"] = 0.03125 / 2
	...
	data.raw["recipe"]["concrete-recycling"]["energy_required"] = 0.0625

	local RECYCLE_TO_ITSELF = {
		{ "solid-fuel", 0.03125 }, { "steel-plate", 1 }, { "copper-plate", 0.2 }, { "iron-plate", 0.2 },
		{ "plastic-bar", 0.0625 }, { "holmium-plate", 0.0625 }
	}
	...
	if mods["Krastorio2"] then
		...
		data.raw["recipe"]["electronic-circuit-recycling"]["results"] = {
			{ type = "item", name = "iron-plate", amount = 0, extra_count_fraction = 0.25 },
			{ type = "item", name = "copper-cable", amount = 0, extra_count_fraction = 0.5 }
		}
		data.raw["recipe"]["electronic-circuit-recycling"]["energy_required"] = 0.0125

		if not leave_higher_chip_recycling_unchanged then
