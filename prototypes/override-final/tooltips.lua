...
for _, quality in pairs(data.raw.quality) do
	local player_heater_radius = math.min(
		common.MAX_HEATING_RADIUS, common.FULGORAN_RADIATIVE_TOWER_HEATING_RADIUS_PLAYER + quality.level
	)

	radiative_heater.custom_tooltip_fields[1].quality_values[quality.name] = {
		"cerys.metres-tooltip-value", tostring(player_heater_radius)
	}
