...
function Public.get_tiles_in_circle(radius,center)
    if radius <= 1 then
        return {{x = math.ceil(center.x-0.5), y = math.ceil(center.y-0.5)}}
    end
    ...
    for x = -limit+center.x, limit+center.x,1 do
        for y = -limit+center.y, limit+center.y do
            -- Find closest point on this square to the origin.
            local dx = math.max(math.abs(x-center.x) - 0.5, 0)
            local dy = math.max(math.abs(y-center.y) - 0.5, 0)
...
function Public.get_tile(center)
    return {{x = math.ceil(center.x-0.5), y = math.ceil(center.y-0.5)}}
end
